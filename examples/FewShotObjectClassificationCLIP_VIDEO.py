...
def detect_and_classify(Image = None, productos_classes_embeddings = None):
    ...
    boxes, _, _ = gft_model.detect_boxes_with_filters_Image(Image, imgsz=480, 
                                                                     conf=0.7, iou=0.9, 
                                                                     confidence_threshold=0.7,
                                                                     min_height=200, max_height=800,
                                                                     min_width=120, max_width=1000,
                                                                     min_aspect_ratio=0.25, max_aspect_ratio=2.6,
                                                                     min_y1=150, max_y1=1600)
...
# Cargar Video
cap = cv2.VideoCapture('./frames/videos/lab/video4.mp4')
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        if contador_fotogramas % saltar_fotogramas == 0:
            ...
            cv2.imshow('Frame Reducido', frame_reducido)
