...
class DetectAndClassifySigLIP:
    def __init__(self, model_path):
        ...
        self.model_SigLIP = SiglipVisionModel.from_pretrained("google/siglip-base-patch16-224")
    ...
    def preprocess(self, image):
        # Reemplaza esta función con el procesamiento adecuado si es necesario
        ...
        for class_folder in class_folders:
            ...
            for img_file in os.listdir(class_path):
                if img_file.lower().endswith(('.jpg', '.png', '.bmp', '.tif', '.tiff')):
                    ...
                    with torch.no_grad():
                        outputs = self.model_SigLIP(**inputs.to(self.device))
                        img_embedding = outputs.pooler_output

                    # Guardar el embedding en formato .pt
                    embedding_file_path = os.path.join(embeddings_folder, img_file.rsplit('.', 1)[0] + '.pt')                    
                    torch.save(img_embedding, embedding_file_path)

        print("Embeddings calculados y guardados con éxito.")

    def load_embeddings(self, classes_path):
        class_folders = [f for f in os.listdir(classes_path) if not f.startswith('.') and os.path.isdir(os.path.join(classes_path, f))]
        embeddings = {}
        for class_folder in class_folders:
            class_path = os.path.join(classes_path, class_folder)
            ...
            for embedding_file in os.listdir(class_path):
                if embedding_file.lower().endswith('.pt'):
                    embedding_file_path = os.path.join(class_path, embedding_file)
                    img_embedding = torch.load(embedding_file_path)
                    embeddings[class_folder].append(img_embedding)
    ...
    def compute_bboxes_embeddings(self, cropped_images):
        ...
        preprocessed_images_tensors = [self.preprocess(Image.fromarray(cv2.cvtColor(crop, cv2.COLOR_BGR2RGB))) for crop in cropped_images]
        
        # SigLIP espera una lista de tensores para procesar varios inputs, por lo que concatenamos los tensores a lo largo de la primera dimensión (batch dimension)
        batch = {key: torch.cat([d[key] for d in preprocessed_images_tensors], dim=0).to(self.device) for key in preprocessed_images_tensors[0]}

        self.model_SigLIP.to(self.device)
        with torch.no_grad():
            outputs = self.model_SigLIP(**batch)
            embeddings = outputs.pooler_output
        return embeddings

    def classify_crops_with_knn(self, crops, embeddings, k=5):

        crops_embeddings = self.compute_bboxes_embeddings(crops).to('cpu').numpy()
        ...
        for class_name, class_embeddings in embeddings.items():
            for embedding in class_embeddings:
                all_class_embeddings.append(np.array(embedding.to('cpu')))
